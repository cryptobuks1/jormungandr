...
impl Block {
    ...
    pub async fn date(&self, context: &Context<'_>) -> FieldResult<BlockDate> {
        self.get_explorer_block(&extract_context(&context).await.db)
            .await
            .map(|b| b.date().into())
    }
    ...
    pub async fn transactions(
        ...
        let explorer_block = self
            .get_explorer_block(&extract_context(&context).await.db)
            .await?;
...
impl Query {
    ...
    pub async fn tips(&self, context: &Context<'_>) -> Vec<Branch> {
        extract_context(&context)
            .await
            .db
            .get_tips()
    ...
    /// tip
    async fn main_tip(&self, context: &Context<'_>) -> Branch {
        let (hash, state_ref) = extract_context(&context).await.db.get_main_tip().await;
